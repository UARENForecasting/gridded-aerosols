...
c
c     old compilation:
c     ifort -convert big_endian -I/usr/include write_aerosols_for_metgrid_mod.f -L/st1/luong/lib/netcdf-4.3.3.1/lib -lnetcdf -lnetcdff -o write_aerosols_for_metgrid_mod.Linux
...
c                              !       to 'SWCORNER' or 'CENTER  '
     ...
     $'GEOS.fp.fcst.inst1_2d_hwl_Nx.20180604_00+20180613_0600.V01.nc4'
      ...
      llxmap = -179.5
      llymap = -89.5
      deltax = 1. !!! 0.3125  ! 360 / 0.3125 =  1152, lon = -180 to 179.6875
      deltay = 1. !!! 0.25    ! 180 / 0.25 + 1 = 721, lat = -90 to 90.
      ...
      STATUS = NF_INQ_VARID (NCID, 'TOTEXTAU', AODID)
      IF (STATUS .NE. NF_NOERR) CALL HANDLE_ERR(STATUS)
      STATUS = NF_INQ_VARID (NCID, 'TAUSTDEV', ANGID)
      print *,'here 00'
      IF (STATUS .NE. NF_NOERR) CALL HANDLE_ERR(STATUS)
...
c     1) WRITE FORMAT VERSION
      ounit = 11
      outfilename = 'ICAP.FILE:'//hdate(1:13)
